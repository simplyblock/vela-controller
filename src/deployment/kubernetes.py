...
class KubernetesService:
    def __init__(self):
        ...
        try:
            ...
            logger.info("Kubernetes client configured with in-cluster config")
        except config.config_exception.ConfigException:
            try:
                # Fall back to kubeconfig file (for local development)
                config.load_kube_config()
                self.core_v1 = client.CoreV1Api()
                logger.info("Kubernetes client configured with kubeconfig file")
            except config.config_exception.ConfigException:
                logger.warning("Kubernetes client not configured: no in-cluster or kubeconfig found")
    ...
    def check_namespace_status(self, namespace):
        ...
        if self.core_v1 is None:
            return {
                ...
                'status': 'unknown',
                'pods': [],
                'message': 'Kubernetes client not configured. Mount kubeconfig or run in-cluster.',
            }
        try:
