...
class RoleUserLink(AsyncAttrs, SQLModel, table=True):
    role_id: ULID = Field(foreign_key="role.id", primary_key=True, sa_type=DBULID)
...
async def _lookup(session: SessionDep, organization: OrganizationDep, role_id: ULIDType) -> Role:
    try:
