...
async def _get_usage(session: SessionDep, filter_, usage_cycle) -> ResourceLimitsPublic:
    query = (
        select(col(ResourceUsageMinute.resource), func.sum(ResourceUsageMinute.amount).label("amount"))
        .where(filter_)
    ...
    return _list_to_resources((await session.execute(query)).all(), "amount")
