...
kubectl -n kube-system \
  ...
  autoscale-scheduler=docker.io/simplyblock/autoscaling:autoscale-scheduler-main-6afdc9f
...
kubectl -n kube-system \
  ...
  autoscaler-agent=docker.io/simplyblock/autoscaling:autoscaler-agent-main-6afdc9f
...
kubectl -n neonvm-system \
  ...
  vxlan-controller=docker.io/simplyblock/autoscaling:neonvm-vxlan-controller-main-6afdc9f
kubectl -n neonvm-system rollout status daemonset/neonvm-vxlan-controller

kubectl -n neonvm-system \
  set image deployment/neonvm-controller \
  manager=docker.io/simplyblock/autoscaling:neonvm-controller-main-6afdc9f
kubectl -n neonvm-system rollout status deployment/neonvm-controller

kubectl -n neonvm-system \
  set image daemonset/neonvm-runner-image-loader \
  neonvm-runner-loader=docker.io/simplyblock/autoscaling:neonvm-runner-main-6afdc9f
...
kubectl -n neonvm-system set env deployment/neonvm-controller \
  VM_RUNNER_IMAGE=docker.io/simplyblock/autoscaling:neonvm-runner-main-6afdc9f
kubectl -n neonvm-system rollout status deployment/neonvm-controller
```
